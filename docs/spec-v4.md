...
[^2]: **v1 디자인 개정 추가.** 사용자가 참고 제품 사진을 보고 "다이얼이 표시되는 부분(원판) 외의
  ...
  항상 어둡다(`--ft-chassis-bg` 기본값 `#0b0b0e`). 동시에 6테마×2게이지 12조합을 위젯 안에서 바로
  전환할 수 있는 컨트롤(테마 스와치 6개 + 게이지 스타일 토글 1개)을 추가했다 — HTML 속성
  (`theme`/`gauge`)만으로 정적으로 고르던 것을, 실행 중에도 클릭 한 번으로 바꿀 수 있게 한 것.
