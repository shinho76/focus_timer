...
- **리셋 버튼**: 시작 버튼 바로 옆. 실행/일시정지 중인 타이머를 즉시 idle 로 되돌린다 — 저빈도
  "설정"이 아니라 상시 필요한 복구 동작이라 설정 패널이 아닌 여기 둔다(v1.4, 사용자 피드백:
  "리셋을 다시 찾기가 어려워").
...
**아이콘 뒤에 접힌 것** (다이얼 아래, 미세 조정 아래) — 둘 다 44px 스퀴클(둥근 사각) 버튼에
"목표"/"설정" 캡션이 항상 붙어 있다:
