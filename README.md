...
| `autostart-on-release` | `on` \| `off` | `on` | 프리셋 버튼 클릭 시 자동 시작할지(v1.6부터 다이얼 클릭/드래그에는 적용 안 됨) |
...
- **전송(transport) 버튼 2개**: 카세트 플레이어처럼 생긴 아이콘으로, 같은 폭 나란히(v1.6).
  이모지가 아니라 `currentColor` 를 따르는 자체 SVG 아이콘이다(v1.7) — 컬러 이모지는 플랫폼
  폰트가 색을 고정시켜(예: 재생 삼각형이 파란색) 전체 디자인 색(검정/중립색)에 맞출 수 없었다.
  - 재생/일시정지/정지 아이콘 (시작/일시정지/재개/확인 — 하나의 버튼, 상태에 따라 아이콘만
    바뀐다). 접근성 이름은 `aria-label`로 "시작"/"일시정지"/"재개"/"확인" 그대로 유지.
  - 되감기 아이콘 = 리셋 — 실행/일시정지 중인 타이머를 즉시 idle 로 되돌린다. 저빈도 "설정"이
    아니라 상시 필요한 복구 동작이라 설정 패널이 아닌 여기 둔다(v1.4, 사용자 피드백: "리셋을
    다시 찾기가 어려워").

**시계 중앙을 직접 눌러도** 시작/일시정지/재개/확인이 된다(v1.6) — 리드아웃(MM:SS) 위에 투명
버튼이 겹쳐 있어 시간 숫자를 가리지 않으면서 같은 동작을 한다. 완전히 안 보이면 눌러도 되는지
알 수 없다는 피드백을 받아, 허브 아래쪽에 아주 옅은 반투명 아이콘(재생/일시정지/정지, 같은
상태를 따라간다)을 얹어 최소한의 힌트를 남겼다(v1.7).

**아이콘 뒤에 접힌 것** (다이얼 아래) — 셋 다 44px 스퀴클(둥근 사각) 버튼에 캡션이 항상
붙어 있다:

- **조정 버튼(중립색)**: "미세 조정" 패널을 연다 — ± 버튼 6개(−10/−5/−1/+1/+5/+10)와 분 직접
  입력. 프리셋만으로는 부족한 미세 조정을 위한 것이라 기본은 접혀 있다. running 중에는 다이얼
  대신 이 ± 버튼들로 남은 시간을 조정한다.
- **`+` 버튼(액센트색)**: 오늘의 목표 입력창을 연다 (아래 문단 참고). 자주 쓰는 진입점이라
  강조색을 유지했다.
- **톱니 버튼(중립색)**: 설정 패널을 연다 — **테마 세그먼트 6개**(`auto`/`classic`/`purple`/`pink`
  /`sky`/`dark`, 텍스트 없이 버튼 전체를 그 테마 색으로), **게이지 스타일 세그먼트 2개**("세그먼트"
  = 점선 링 아이콘/`gauge="segments"`, "파이 차트" = 꽉 찬 원 아이콘/`gauge="sector"`), **알람 미리
  듣기**. 전부 저빈도 기능이라 목표 버튼과 달리 중립색으로 낮춰 우선순위를 구분했다.
...
자리가 가리키는 시간으로 바로 점프해 커밋된다. 그대로 끌면 그 지점부터 상대 회전이 이어진다
(자세한 동작은 `docs/spec-v4.md` §15.1). **단, 자동 시작은 하지 않는다**(v1.6, 사용자 피드백
반영 — §15.1 도입 당시엔 프리셋과 동등하게 autostart 했지만 "시간만 바꾸려는데 바로 시작돼
버린다"는 피드백으로 되돌렸다) — 값만 바뀌고, 시작은 항상 전송 버튼이나 시계 중앙 탭으로만
한다. 프리셋 버튼은 여전히 `autostart-on-release` 속성을 따른다(기본 on, 다이얼에는 더 이상
적용되지 않는다).

**일시정지 중에도 다이얼(과 프리셋·±버튼·숫자 입력)로 재조정할 수 있다**(v1.7, 사용자
피드백) — 손대는 순간 그 일시정지 세션은 버려지고 새로 설정하는 모드로 들어간다(저장된 진행
기록도 함께 지워진다). **실행(running) 중에는 여전히 다이얼 조작이 거부된다** — 원 스펙의
"실행 중 다이얼 조작 거부"(§2.2)를 그대로 지킨다. 실행 중 남은 시간을 조정하려면 ± 버튼을
쓴다(§2.3, 다이얼 대신 남은 시간을 직접 가감).
...
```bash
npm test          # Vitest 단위 테스트 (309개, core/view/input/ports/runtime 전부)
