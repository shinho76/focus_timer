...
- **전송(transport) 버튼 2개**: 카세트 플레이어 이모지로, 같은 폭 나란히(v1.6).
  - ▶️/⏸️/▶️/⏹️ (시작/일시정지/재개/확인 — 하나의 버튼, 상태에 따라 아이콘만 바뀐다). 접근성
    이름은 `aria-label`로 "시작"/"일시정지"/"재개"/"확인" 그대로 유지.
  - ⏮️ 리셋 — 실행/일시정지 중인 타이머를 즉시 idle 로 되돌린다. 저빈도 "설정"이 아니라 상시
    필요한 복구 동작이라 설정 패널이 아닌 여기 둔다(v1.4, 사용자 피드백: "리셋을 다시 찾기가
    어려워").

**시계 중앙을 직접 눌러도** 시작/일시정지/재개/확인이 된다(v1.6) — 리드아웃(MM:SS) 위에 투명
버튼이 겹쳐 있어 시간 숫자를 가리지 않으면서 같은 동작을 한다.
